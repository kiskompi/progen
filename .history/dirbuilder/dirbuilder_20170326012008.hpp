...
public:
    DirBuilder();
    DirBuilder(std::string const structure_str);
    ~DirBuilder();

    /// Reads a file to the 'structure' variable by calling the function of FTree
    /// responsible for this task
    static std::vector<std::string> read_file(std::string const fname);
    ...
    void create_dirs_with_path(std::string const path) const;
