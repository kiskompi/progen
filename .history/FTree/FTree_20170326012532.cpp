...
FTree::Folder::Folder(std::shared_ptr<Folder> const parent_, std::string const path_)
    :depth(0),path(path_),parent(parent_)
{
    std::string tmp = "aaa";
    std::copy(tmp.begin(), tmp.end(), this->perm.data());
}

FTree::Folder::Folder(std::shared_ptr<Folder> const parent_, std::string const path_, int const depth_, std::string const perm_)
...
void FTree::create_dirs(){
    create_dirs(std::make_unique<Folder>(root));
}

void FTree::create_dirs(std::unique_ptr<Folder> const act) const
{
    ...
    if (!act->childs.empty()) {
        ...
        for(int i = 0; i<act->childs.size(); ++i) {
            ...
            create_dirs(std::make_unique<Folder>(act->childs.at(i)));
        }
    }
}
...
void FTree::add_child(Folder const &add_this)
{
    ...
    //std::shared_ptr<Folder> act = std::make_shared<Folder>(current);
    //std::cout<<(act->childs.at(act->childs.size()-1)).path<<std::endl;
    
}

void FTree::build(std::vector<std::string> const content)
{
    std::string line;
    int prev_depth = -1;
    size_t checker = 0;
