...
FTree::Folder::Folder(std::shared_ptr<Folder> parent_, std::string path_,
                      int depth_, std::string perm_)
...
FTree::FTree()
    :root(Folder()), current(std::make_unique<Folder>(root))
{
}
...
void FTree::create_dirs()
{
    ...
    if (current->childs.empty()) {
        ...
        for(int i = 0; i<current->childs.size(); ++i) {
            ...
            current = std::make_unique<Folder>(current->childs.at(i));
    ...
    for (auto const &line : content) {
        ...
        try {
            if(depth == prev_depth + 1){
                ...
                if (!current->childs.empty()) 
                    current = std::make_unique<FTree::Folder>(current->childs.at(current->childs.size()-1));
...
void FTree::explore(void (*func)())
{
    ...
    current = std::make_unique<Folder>(FTree::Folder(root));
...
void FTree::explore(void (*func)(), FTree::Folder folder)
{
    ...
    current = std::make_unique<Folder>(FTree::Folder(folder));
    for(int i = 0; i<root.childs.size(); ++i) {
        Folder &tmp = (current->childs[i]);
        this->explore(func, tmp);
    }
