...
/// In-memory data structure for storing folder structures. Built for progen.
    ...
    struct Folder {
        ...
        Folder(std::unique_ptr<Folder> parent_, std::string path_);
        ...
        Folder(std::unique_ptr<Folder>, std::string, int depth, std::string perm);
        ~Folder();
    };

    Folder root;                                        ///< if path is given, it is given to root so its path is not . anymore
    std::unique_ptr<Folder> current;                    ///< pointer to the currently (or last) used node
