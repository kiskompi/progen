...
public:
    struct Folder {
        ...
        Folder(std::shared_ptr<Folder> const parent_, std::string const path_);
        ...
        Folder(std::shared_ptr<Folder> const parent_, std::string const path_, int const depth_, std::string const perm_);
        ~Folder();
    };
...
public:
    ...
    void create_dirs(std::shared_ptr<Folder> const act) const;
    void create_dirs();
    ...
    void build(std::vector<std::string> const dotstr_content);

    /// Sets the path of the 'root' node to a given path.
    void set_path(std::string new_path);

    /// Builds a single string from a stack of strings
    static std::string stack_to_string(std::stack<std::string> string_stack) const;
};
