...
void FTree::add_child(Folder const &add_this)
{
    ...
    for (Folder& i: current->childs) std::cout<<"Cur: "<<current->path<<"Chld: "<<i.path<<std::endl;
}
    ...
    for (auto const &line : content) {
        ...
        try {
            if(depth == prev_depth+1)
            {
                ...
                path_stack.pop();
                path_stack.push(name);
                current = std::make_shared<FTree::Folder>(current->parent.at(current->childs.size()-1));
